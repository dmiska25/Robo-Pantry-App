...
describe("ProductListing page", () => {
  ...
  describe("when products are recieved", () => {
    ...
    beforeAll(() => {
      spy.mockReturnValue([false, productsMock, null]);
    });
    afterAll(() => {
      spy.mockReset();
    });
    ...
    it("displays correct listing information", () => {
      ...
      results.forEach((productListing, i) => {
        ...
        expect(productListing.props.href).toBe(
          `/Product%20Details?itemId=${productsMock[i].id}`
        );
      });
    });
  ...
  describe("when error is not null", () => {
    beforeAll(() => {
      spy.mockReturnValue([false, null, "error loading page"]);
    });
    afterAll(() => {
      spy.mockReset();
    });
  ...
  describe("when 0 products are returned", () => {
    beforeAll(() => {
      spy.mockReturnValue([false, [], null]);
    });
    afterAll(() => {
      spy.mockReset();
    });
